...
class Third_page:
    ...
    def insert_data(self):
        ...
        try:
            self.cur.execute("CREATE TABLE DATA03 (id SERIAL PRIMARY KEY,BOARD TEXT,DEGREE TEXT,EXAM_TYPE TEXT,PASSING_YEAR TEXT,ROLL_NO TEXT,REGESTRATION_NO TEXT,TOTAL_MARKS TEXT,OBTAINED_MARKS TEXT,GRADE TEXT,DIVISION TEXT)")
        except:
            self.add_data()
        else:
            self.add_data()
    def add_data(self):
        ...
        self.cur.execute(f"INSERT INTO DATA03 VALUES {self.tupe}")
        self.con.commit()
        pass
    def hsoe(self):
        """This method will printn the data from the table but only for the admin use please"""
        self.cur.execute(f"SELECT * FROM DATA03")
        r = self.cur.fetchall()
        for i1 in r:
            print()
            for j in i1:
