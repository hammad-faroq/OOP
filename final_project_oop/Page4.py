...
class Last_page:
    ...
    def data(self):
        """This is behind the sceen working of the button sHow.We pressed it will fetch data from all the tables and then creates new labesl in a loop to show the data """
        ...
        for i in range(record_no, record_upto + 1):
            ...
            try:
                ...
                label1.pack()
                self.cur.execute(f"SELECT * FROM DATA03 WHERE id = ?", (i,))
                self.j = self.cur.fetchall()
                self.r = self.j[0]
